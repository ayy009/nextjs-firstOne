    ...
    <>
      <div className=" grid grid-cols-1 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-4 2xl:gap-7.5">
        {dataStatsList.map((item, index) => (
          <div
            key={index}
            onClick={e=>{console.log(item.color)}}
            // className={`rounded-[10px] bg-white p-6 shadow-1 dark:bg-gray-dark `}
            // style={{
            //   backgroundColor: isHovered ? item.color : 'white', // Apply dynamic color on hover
            // }}
            // style={{ backgroundColor: item.color }}
            // className={`rounded-[10px] bg-white p-6 shadow-1 dark:bg-gray-dark dark:text-white hover:bg-[${item.color}] dark:hover:bg-[${item.color}] hover:text-gray-950`}
            className={` rounded-[10px] hover:bg-[${item.color}] bg-white p-6 shadow-1 dark:bg-gray-dark dark:text-white  hover:text-gray-950`} 
          >
            <div
